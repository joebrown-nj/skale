...
final class LeadFormRequest
{
    ...
    public function server(): array { return $this->request->server(); }
    public function clientIp(): string { return $this->request->clientIp(); }

    /** @return list<string> */
    public function errors(): array
    {
        $errors = [];
        if ($this->data['name'] === '') $errors[] = 'Name is required';
        if (!filter_var($this->data['email'], FILTER_VALIDATE_EMAIL)) $errors[] = 'Email is required';
        if (!$this->csrfIsValid()) $errors[] = 'The form has expired. Please refresh and try again.';
        return $errors;
    }
    ...
    private function csrfIsValid(): bool
    {
        ...
        if (!is_string($expected) || $expected === '') return true;
