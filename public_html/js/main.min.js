...
$(document).on('click','.mbtn',function(t){const url=$(this).attr('href')=='/'?'':new URL($(this).attr('href'));const slug=url.pathname;const searchString=url?url.search:'';const queryString=searchString.toString().replace('?','');ajaxGetPageContent(slug,queryString,t);bsOffcanvas.hide();return!1});function setActiveNavItem(url){$('#oCNav .active').removeClass('active');var adjustedUrl=url=='/'||url==''?url:url.replace('/','').replaceAll('/','-');adjustedUrl=adjustedUrl.trim();if(url!='/')
...
$.ajax({type:'GET',url:'/meta-data/'+encodeURIComponent(slug),success:function(data){if(data){var json=$.parseJSON(data);$('meta[name=description]').attr('content',json.description);$('meta[name=keywords]').attr('content',json.keywords);$('head title').text(json.title);$('link[rel="canonical"]').attr('href',window.location.href);$('meta[property="og:description"]').attr('content',json.description);$('meta[property="og:type"]').attr('content',window.location.href.includes('/blog/')?'article':'website');$('meta[property="og:title"]').attr('content',json.title);$('meta[property="og:URL"]').attr('content',window.location.href)}}})});return!1}
