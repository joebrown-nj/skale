...
$(document).on('click','.mbtn',function(t){const url=!t.target.href?'':new URL(t.target.href);var slug=!t.target.pathname?'/':t.target.pathname;const interests=!url?'':url.searchParams.get("interests");const queryString=interests?'interests='+interests:'';ajaxGetPageContent(slug,queryString,t);bsOffcanvas.hide();return!1});function setActiveNavItem(url){$('#oCNav .active').removeClass('active');var adjustedUrl=url=='/'||url==''?url:url.replace('/','').replaceAll('/','-');adjustedUrl=adjustedUrl.trim();if(url!='/')
...
setActiveNavItem(slug);$.ajax({type:'GET',url:slug+'?header=false&footer=false'+(queryString&&queryString.length>0?'&'+queryString:''),success:function(data){$('.page-content').html(data);$(window).scrollTop(0)}}).done(function(){if(!slug||slug.length==0||slug=='/'){$('header.menu-bar').removeClass('menu-bar-bg')}else{$('header.menu-bar').addClass('menu-bar-bg')}
$.ajax({type:'GET',url:'/meta-data?slug='+encodeURIComponent(slug),success:function(data){if(data){var json=$.parseJSON(data);$('meta[name=description]').attr('content',json.description);$('meta[name=keywords]').attr('content',json.keywords);$('head title').text(json.title);$('link[rel="canonical"]').attr('href',window.location.href);$('meta[property="og:description"]').attr('content',json.description);$('meta[property="og:type"]').attr('content',window.location.href.includes('/blog/')?'article':'website');$('meta[property="og:title"]').attr('content',json.title);$('meta[property="og:URL"]').attr('content',window.location.href)}}})});return!1}
...
if(data.success&&data.success.length>0){document.getElementById("contactForm").reset();$('#contactForm').prepend('<div class="alert alert-success" role="alert">'+data.success+'</div>')}}}).done(function(){hideOverlay()})}
