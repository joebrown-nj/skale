...
$(document).on('click','.mbtn',function(t){const url=!t.target.href?'':new URL(t.target.href);const slug=!t.target.pathname?'/':t.target.pathname;const queryString=url.search.replace('?','');ajaxGetPageContent(slug,queryString,t);bsOffcanvas.hide();return!1});function setActiveNavItem(url){$('#oCNav .active').removeClass('active');var adjustedUrl=url=='/'||url==''?url:url.replace('/','').replaceAll('/','-');adjustedUrl=adjustedUrl.trim();if(url!='/')
